...
exports.SignUp = tryCatch(async (req, res) => {
  ...
  const user = await User.create({
    ...
    photo:
      'https://res.cloudinary.com/ult-bank/image/upload/v1685139259/t9tjkxnb3nrmtjuizftp.png',
  });
...
exports.Login = tryCatch(async (req, res) => {
  ...
  if (!mobile || !password) {
    ...
    throw new AppError(
      400,
      'your phone number is not verified, please go to your eamil and verify',
      400
    );
  } else {
    if (rememberme) {
...
exports.forgotPassword = tryCatch(async (req, res, next) => {
  try {
    ...
    if (user.verified === false) {
      throw new AppError(
        'Unauthorized',
        'Sorry this account is not verified, verify to contd',
        401
      );
    }
