...
exports.reportGeneration = tryCatch(async (req, res) => {
  const totalJobsInProgress = await Job.find().where({
    jobStatus: 'in progress',
  });
  const totalJobsOrders = await Job.find();
  const totalAmt = await Job.find()
    .select('amount')
    .where({ jobStatus: 'completed' });
  const totalAmtInProgress = await Job.find()
    .select('amount')
    .where({ jobStatus: 'in progress' });
  const totalAmtPending = await Job.find()
    .select('amount')
    .where({ jobStatus: 'pending' });
  const clients = await Job.find().select([
    'client_name',
    'project_type',
    'job_title',
    'amount',
